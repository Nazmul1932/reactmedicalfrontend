...
class ManageMedicineComponent extends React.Component {
    constructor(props) {
        super(props);
        this.formSubmit = this.formSubmit.bind(this);
    }
    state = {
        errorRes: false,errorMessage: "", btnMessage: 0,sendData: false,companylist: [],
        medicinedetails: [],
        MedicineDataList:[],dataLoaded: false,name: "", medical_typ: "", buy_price: "",
        sell_price: "",c_gst: "", s_gst: "", batch_no: "",shelf_no: "",expire_date: "",
        mfg_date: "",company_id: "", description1: "",in_stock_total: "",qty_in_strip: "",
        total_salt_list: 0,medicine_id: 0,
    };
    ...
    async formSubmit(event) {
        ...
        var response = await apiHandler.saveMedicineData(
            ...
            event.target.mfg_date.value,event.target.company_id.value,event.target.description1.value,
            event.target.in_stock_total.value,event.target.qty_in_strip.value,this.state.medicinedetails,
    ...
    async LoadInitialData() {
        ...
        var medicine_data = await apihandler.FetchMedicineAll();
        this.setState({ companylist: companydata.data });
        this.setState({MedicineDataList:medicine_data.data.data});
                                    ...
                                    <table className="table table-hover">
                                        ...
                                        {this.state.MedicineDataList.map((medicine, index)=>
                                            <tr key={medicine.id}>
                                                ...
                                                <td>
                                                    <button className="btn btn-block btn-warning" onClick={()=>this.ViewMedicineDetails(index)}>
                                                        View
                                                    </button>
                                                </td>
                                            </tr>
